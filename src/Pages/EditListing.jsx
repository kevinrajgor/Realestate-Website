...
export default function CreateListing() {
  ...
  function onChange(e) {
    ...
    if (!e.target.files) {
      setFormData((prev) => ({
        ...prev,
        [e.target.id]: boolean ?? e.target.value, //? for if the value is null
      }));
    }
  }
  async function onSubmit(e) {
    ...
    if (geoLocationEnabled) {
      const response = await fetch(
        `https://api.tomtom.com/search/2/geocode/${address}.json?key=${process.env.REACT_APP_GEOCODE_API_KEY}`,
      );
